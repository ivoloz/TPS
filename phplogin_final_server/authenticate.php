...
// Set var for changing password; Will be overwritten if password is not initial anymore!
if ($stmt = $con->prepare('SELECT benutzerid, rollenid, passwort, init_passwort FROM benutzer WHERE email = ?')) {
	...
	if ($stmt->num_rows > 0) {
	$stmt->bind_result($benutzerid, $rollenid,$passwort, $init_passwort);
	$stmt->fetch();
