...
if ($stmt = $con->prepare('INSERT INTO benutzer (arbeitgeberid, rollenid, vorname,nachname, email, passwort,kaz_von,kaz_bis, max_gesamtstunden,max_ueberstunden, init_passwort) VALUES ( ?,?,?, ?, ?,?,?,?,?,?,?)')) {
	...
	$kaz_von=null;
	$kaz_bis=null;
	$max_gesamtstunden=null;
	$max_ueberstunden=null;

   $stmt->bind_param('sssssssssss', $arbeitgeberid, $rollenid, $_POST['vorname'], $_POST['nachname'],$_POST['email'], $pwhashed,$kaz_von,$kaz_bis,$max_gesamtstunden,$max_ueberstunden , $pwhashed );
	$stmt->execute();
try {
